...
const std::pair<uint32_t, TimeTable::EventData> emptyEventEntry = {0, {GlobalEnum::ScheduleAction::INVALID, Value()}};
...
TimeTable::TimeTable(std::vector<ActionData>&& actionList)
{
...
{
    EventData entry;
    entry.action = GlobalEnum::ScheduleAction::WRITE;
    entry.value = value;
    ...
    if (upperBound->second.action != GlobalEnum::ScheduleAction::UNSET)
    {
        eventMap[daySecond] = entry;
        printf("Write added.\n");
        return true;
...
{
    EventData beginner;
    beginner.action = GlobalEnum::ScheduleAction::SET;
    beginner.value = value;
    eventMap[beginSecond] = beginner;
    EventData ender;
    ender.action = GlobalEnum::ScheduleAction::UNSET;
    //Call the unset the next second
    eventMap[endSecond + 1] = ender;
